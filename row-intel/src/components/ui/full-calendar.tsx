...
import SubmitPlanWorkout from "../SubmitPlanWorkout";
...
type ContextType = {
  ...
  setWorkouts: (workouts: PlannedWorkout[]) => void;
  activePlanId: string;
...
const Calendar = ({
  ...
  const [workouts, setWorkouts] = useState<PlannedWorkout[]>(defaultWorkouts);
      ...
      {events.map((event) => {
        ...
        return (
          <div
            key={event.id}
            className={cn(
              "relative overflow-hidden h-20 mb-4 border hover:border-primary-lightBlue hover:bg-primary-lightBlue/50 cursor-pointer hover:text-primary-white",
      ...
      <div className="flex sticky top-0 items-center z-10 border-b mb-3">
        ...
        {headerDays.map((date) => (
          <div
            key={date.toString()}
            className={cn(
              "text-center flex-1 gap-1 pb-2 text-sm flex items-center justify-center text-gray-600"
            )}
          >
            ...
            <span
              className={cn(
                ...
                isToday(date) &&
                  "bg-primary-darkBlue text-primary-foreground rounded-full size-6"
              )}
            >
              {format(date, "d")}
            </span>
        ...
        <div className="grid grid-cols-7 flex-1">
          {weekDates.map((day, i) => {
            const eventsForDay = events.filter((event) =>
              isSameDay(event.start, day[i])
            );
            return (
              <div
                className={cn(
                  "h-full text-sm text-muted-foreground border-l last:border-r"
                )}
                key={day.toString()}
              >
                <EventGroup
...
const CalendarMonthView = () => {
  const { date, view, events, locale, workouts, activePlanId } = useCalendar();
  console.log("Workouts from calendar view: ", workouts);
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="grid grid-cols-7 gap-px sticky top-0  border-b">
        {weekDays.map((day, i) => (
          <div
            key={day}
            className={cn(
              "mb-2 text-left pl-2 text-sm text-gray-500 ",
      ...
      <div className="grid overflow-hidden -mt-px flex-1 auto-rows-fr p-px grid-cols-7 gap-px">
        {monthDates.map((_date) => {
          ...
          return (
            <div
              className={cn(
                "ring-1 p-1 text-sm text-muted-foreground ring-border overflow-auto flex flex-row gap-2",
            ...
            >
                ...
                {currentEvents.map((event, i) => {
                  return (
                    <SubmitPlanWorkout
                      key={event.id}
                      plannedWorkout={
                        workouts.find((workout) => workout.id === event.id) ||
                        workouts[i]
                      }
                    ...
                    >
                      <div
                        key={event.id}
                        className="px-1 rounded text-sm flex items-center border border-primary-grey gap-1 hover:border-primary-lightBlue hover:bg-primary-lightBlue/50 cursor-pointer"
                      >
                        <div
                          className={cn(
                            "shrink-0",
              ...
              <span
                className={cn(
                  "size-6 grid place-items-center rounded-full mb-1 sticky top-0 ml-auto",
                  isToday(_date) &&
                    "bg-primary-darkBlue text-primary-foreground"
                )}
              >
                {format(_date, "d")}
              </span>
